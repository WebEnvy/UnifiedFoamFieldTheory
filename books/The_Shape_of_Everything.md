...
## How One Polyhedron Explains All of Physics

**Luke Martin · Sydney, 2026**

---

## Prologue: The Strangest Fact in Science
...
But there is a stronger argument than statistical improbability. The framework also proves a structural theorem: the Standard Model Lagrangian — the complete mathematical description of all known particle physics — is the unique continuum limit of the truncated octahedron lattice.

This theorem has six components. The gauge group SU(3)×SU(2)×U(1) is forced by the symmetry decomposition of the torsion. The three generations of fermions are forced by the three axes of the body-centred cubic lattice. The chirality of the weak force (the fact that it only affects left-handed particles) is forced by the asymmetry between square-face content and hexagonal-face content of the two fermion vibration families — an asymmetry measured by the number one over the square root of 17. The Higgs mechanism (the process that gives particles mass) is forced by the fact that the hexagonal vibration mode has torsion eigenvalue negative one — meaning it is geometrically unstable, and the symmetry must break. Anomaly cancellation (the consistency condition for quantum field theory) is forced by the colour number being three. And CPT invariance (the deepest symmetry in physics) follows from the fact that the symmetry group is a group — every operation has an inverse.
...
If the framework is correct — and that is still an if, because no paper has been peer reviewed and no prediction has been experimentally confirmed — then several things follow.
...
It does not explain why the universe contains more matter than antimatter. The CP violation is derived (it is geometry), but the specific mechanism that converted CP violation into a matter-antimatter asymmetry during the Big Bang is not yet worked out. This is the biggest open physics question in the framework.

The tensor-to-scalar ratio — a specific number describing the amplitude of gravitational waves from the Big Bang — is predicted to be about 0.063. The current best experimental limit is below 0.036. This is a roughly two-standard-deviation tension. Either the prediction is wrong (which would require modifying the cascade amplification argument), or the experimental limit will shift (which is possible — it has shifted before). This tension is reported honestly, not hidden.

No paper in the framework has been peer reviewed. The mathematics is public, the code is available, the predictions are sharp. But the process of having independent experts scrutinise the work has not happened yet. Until it does, the appropriate attitude is interested scepticism.
...
The technical details behind every claim in this book are available in two forms: the research paper series (over 50 papers, available on Zenodo under CC BY 4.0) and the technical monograph "From Foam to Fermions: The Standard Model from the Geometry of One Cell." Both are accessible at github.com/WebEnvy/UnifiedFoamFieldTheory.

The spectral verification script — which computes the eigenvalues of the face Laplacian and verifies every derived quantity — runs in Python in under thirty seconds. It requires no special libraries beyond NumPy.

---

## About

Luke Martin is an independent researcher based in Sydney, Australia. This work was developed in collaboration with Claude (Anthropic). The physical framework, axioms, identifications, and direction are Martin's. The AI contributed computation, derivation, verification, and composition. Every calculation was executed and independently verified. The full conversation record is available on request.
